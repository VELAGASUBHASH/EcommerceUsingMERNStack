...
export const getFeaturedProducts = async(req,res)=>{
    try {
       ...
       if(featuredproducts){
        return res.json(JSON.parse(featuredproducts)); // redis will store product as string
       }

       // if not it will fetch from mongodb
       // .lean() is return a plain javascript object instead of mongodb(good performance)
       ...
       // store Mongodb founded products in redis

       await redis.set("featured_products",JSON.stringify(featuredproducts));
       res.json({featuredproducts});
    } catch (error) {
        console.log("Unable Fetch Products in Redis",error.message);
